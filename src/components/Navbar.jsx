...
function Navbar() {
  ...
  const { currentColor, activeMenu, setActiveMenu, isClicked, screenSize, setScreenSize, setIsClicked, handleClick } = useStateContext();
  useEffect(() => {
      ...
      <div className='flex'>
        ...
        <TooltipComponent content="Profile" position="BottomCenter" >
          <div className="flex items-center gap-2 cursor-pointer p-1 hover:bg-light-gray rounded-lg" onClick={() => handleClick('userProfile')}>
            <img className="rounded-full w-8 h-8" src={avatar} alt="userProfile" />
            <p>
              <span className='text-gray-400 text-14'>Hi,</span> {' '}
              <span className='text-gray-400 font-bold ml-1 text-14'>Yousif</span>
            </p>
